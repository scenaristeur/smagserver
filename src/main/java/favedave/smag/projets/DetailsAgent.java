...
public class DetailsAgent extends Agent {
	...
	public Status activate(Object... parameters) {
		...
		System.out
				.println("Details Agent démarré pour le projet : " + idProjet);
	...
	public Status live() {
		...
		case CONSTRUCTION_REQUETE:
			 ...
			 */
			String queryString = "select * where {<http://smag0.blogspot.fr/ns/smag0#"
					+ idProjet + "> ?propriete ?objet} ";
		...
		case CONSTRUCTION_RESULTAT:
			...
			for (; resultats.hasNext();) {
				...
				// name.
				// Resource projet = soln.getResource("projet") ; // Get a
				// result variable - must be a resource
				// propriete = soln.getProperty() ;
				// Literal objet = soln.getLiteral("objet") ; // Get a result
				// variable - must be a literal
				// RDFNode s=result.get("s");
				RDFNode p = soln.get("propriete");
				RDFNode o = soln.get("objet");

				resultat = resultat + "<tr><td>" + p + "</a></td><td>" + o
						+ "</td></tr>\n";

				/*
				 * i++; JSONObject jresult = new JSONObject();
				 * jresult.put("subject", "sujettest"); jresult.put("predicate",
				 * i); jresult.put("object", o.toString());
				 * System.out.println(jresult.toString());
				 * jsonDetails.put(String.valueOf(i), jresult);
				 */
			}
	 ...
	 * implementation.
	 */
	private class StateChannelImplementation implements
			AgentDetailProjetChannel {
